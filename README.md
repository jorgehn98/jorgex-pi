...
- **Direct package:** first run `engram setup pi` to install and register the official external packages, then install the selected published Pi version explicitly with `pi install npm:jorgex-pi@<published-version>`. If setup is missing, duplicated, malformed, unreadable, or conflicting, Pi fails closed and reports the setup state; it does not install or repair the official packages.
- **Managed Stack:** Stack PR06 will coordinate the same setup and verification later. That managed channel is not published or adopted yet; publishing this Pi package does not ship PR06 or update a Stack candidate.
...
La adopción gestionada de cualquier nueva versión deberá fijar el artefacto exacto y verificar URL, tamaño, SHA-256, SHA-512, lifecycle y rollback cuando Stack PR06 se publique. En este cambio no existe todavía consumo gestionado. La provenance/attestation externa de npm queda fuera de la verificación del runtime.
...
La coordinación downstream con Stack no está publicada en este cambio. No se debe presentar PR06, su artefacto ni su adopción como disponibles; la coordinación gestionada queda para ese PR posterior.
...
La comprobación usa el contrato público `resolveSubagentLaunchContract` de `pi-subagents@0.54.0` en `tests/fixtures/discover-runtime-agents.mjs`. Ese seam permite comprobar la metadata de selección, las rutas/skills resueltas y la allowlist efectiva; no demuestra que un modelo ejecute una skill, que lea el cuerpo completo, que exista una ACL universal ni que todos los runtimes compartan el mismo contrato. El agente `engram` sólo expone seis lecturas nativas de `gentle-engram`: `mem_search`, `mem_context`, `mem_get_observation`, `mem_suggest_topic_key`, `mem_current_project` y `mem_doctor`; no recibe operaciones de escritura, `MCP_DIRECT_TOOLS`, shell ni subdelegación. `subagentOnlyExtensions: ../extensions/engram-child.ts` aplica esa selección al child antes del turno LLM. Esta prueba demuestra la frontera de lectura del child, no una conversación LLM completa ni una adopción gestionada de Engram.
...
The official setup owns the Engram binary, database, and memories. Pi only validates the external configuration needed for the session and never installs, updates, removes, or takes ownership of that state. The child receives only the documented portability and Engram data/project/timezone environment allowlist—not ambient secrets, proxy variables, Node options, cloud autosync credentials, or npm configuration. Passive capture is excluded, so `mem_capture_passive` is never advertised.

The `engram` child is deliberately narrower and read-only: it exposes exactly `mem_search`, `mem_context`, `mem_get_observation`, `mem_suggest_topic_key`, `mem_current_project`, and `mem_doctor` from `gentle-engram`. Save, update, session-write, review, pin, unpin, `MCP_DIRECT_TOOLS`, shell, and subdelegation remain unavailable to that specialist.
