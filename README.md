...
The pinned `@gotgenes/pi-permission-system@27.0.0` companion evaluates the native Pi permission surfaces. JorgeX's generated policy uses `allow` for ordinary workspace reads, writes, edits, search, listing, shell use, skills, subagents, Web Access, and the reviewed Engram and Context7 tool names. Sensitive paths such as `.env`, SSH and AWS credential files, `.npmrc`, Git credentials, private-key filenames, and `*.pem`/`*.key` are denied. Destructive system operations and secret access remain denied; legitimate but sensitive operations, including deletion, privilege escalation, destructive Git actions, wrappers/interpreters, and package-manager execution, require `ask`. Unknown tools use the conservative `ask` fallback. A path rule is transversal, so a stricter path result takes precedence over a tool-level allow.
...
The ownership boundary is `contract/assets.v1.json`. Package-owned external writes include the Sol lifecycle fields in `PI_CODING_AGENT_DIR/settings.json`, `PI_CODING_AGENT_DIR/models.json`, the Sol receipt at `PI_CODING_AGENT_DIR/jorgex-pi/sol-lifecycle.v1.json`, and the permission policy only when JorgeX created that previously absent file. Permission initialization and ownership live in the separate `PI_CODING_AGENT_DIR/jorgex-pi/permissions-lifecycle.v1.json`; cleanup removes only an exact owned policy, retains its backup, and preserves any user replacement. The permission companion's logs and forwarding state, plus ask, Web Access, Goal, and MCP adapter state, remain external state and are not claimed by JorgeX. The manifest records the preserved companion paths that must survive JorgeX install, reinstall, and removal:
