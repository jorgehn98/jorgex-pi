...
- **Directa:** ejecutar primero `engram setup pi` para instalar y registrar los paquetes oficiales externos; después instalar explícitamente la versión publicada con `pi install npm:jorgex-pi@<published-version>`. Pi falla cerrado si el setup falta, está duplicado, malformado, ilegible o en conflicto, y no instala ni repara esos paquetes.
- **Gestionada:** Stack PR06 coordinará ese setup y su verificación más adelante. PR06 todavía no está publicado ni adoptado; publicar Pi no actualiza ni habilita el canal gestionado.
...
La procedencia de la snapshot se consulta en `contract/parity.v2.json`; no repetir aquí SHA. El contrato v2 también registra `assets/system-prompt/AGENTS.md`, `assets/system-prompt/engram-protocol.md`, `assets/system-prompt/context7.md`, los módulos de browser y `prompts/lean-audit.md`. `modular-system-prompts-v1` declara la recomposición marker-aware: Web Access se incluye cuando el bootstrap está sano y no hay conflicto; Context7 se registra en `session_start` mediante el event bus del `pi-mcp-adapter` externo; Playwright solo cuando su handoff resuelve `ready`; DevTools solo cuando el handoff estricto se registra por ese mismo bus. Context7 y DevTools usan el gateway externo; JorgeX no incluye un adapter bundled, no usa `MCP_DIRECT_TOOLS` y no promete handshake ni readiness operativa.
...
El seam verificable incluye el contrato público `resolveSubagentLaunchContract` de `pi-subagents@0.54.0`, usado por `tests/fixtures/discover-runtime-agents.mjs`, que comprueba selección, rutas, metadata y allowlist efectiva, y la prueba de proceso aislado `tests/subagent-engram-child.test.mjs`. El agente `engram` declara `subagentOnlyExtensions: ../extensions/engram-child.ts`; ese shim aplica exactamente seis lecturas nativas de `gentle-engram`: `mem_search`, `mem_context`, `mem_get_observation`, `mem_suggest_topic_key`, `mem_current_project` y `mem_doctor`. No expone `MCP_DIRECT_TOOLS`, escritura, shell ni subdelegación. Esto prueba la frontera de lectura del child, no la ejecución de un modelo, la lectura del cuerpo completo de una skill, una conversación LLM completa, una ACL universal ni la compatibilidad de todos los runtimes. La selección privada de skills de F1 no define los defaults de modelo, permisos, receipts, HOME ni configuración del usuario.
