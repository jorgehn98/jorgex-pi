...
function assertPermissionsProjection(stackDir, modes, projection, projectionExpected) {
  assert.deepEqual(
    Object.keys(projection).sort(),
    ["outputSha256", "sourcePath", "sourceSha256", "targetPath"],
    "permissions projection must record source and generated bytes",
  );
  assert.deepEqual(
    { sourcePath: projection.sourcePath, targetPath: projection.targetPath },
    { sourcePath: projectionExpected.sourcePath, targetPath: projectionExpected.targetPath },
    "permissions projection must match the reviewed canonical paths",
  );
  ...
  const outputPath = join(root, projection.targetPath);
